...
public class PigProcessDatumTest {
    ...
    public void testPigProcessor() throws Exception {
        ...
        String[] inputFields = (String[]) StringUtils.split(input[0], '\t').toArray();
        StreamsDatum inputDatum = new StreamsDatum(inputFields[3]);
        inputDatum.setId(inputFields[0]);
        converter.prepare(null);

        List<StreamsDatum> resultDatums = converter.process(inputDatum);

        String[] output = new String[1];
        output[0] = "(159475541894897679,twitter,statuses/user_timeline,1384499359006," + null + ")";
