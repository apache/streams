...
public class LinkResolver implements Serializable {
    ...
    public ActivityObject getLinkObject() {
        return convertLinkDetailsToActivityObject(this.linkDetails);
    }


    /**
     * Only public for conversions we are going to have to previously indexed data.  Need to remove LinkDetails
     * from LinkResolver in the future.
     * @param linkDetails
     * @return
     */
    public static ActivityObject convertLinkDetailsToActivityObject(LinkDetails linkDetails) {
        ActivityObject linkObject = new ActivityObject();
        linkObject.setId(linkDetails.getFinalURL());
        linkObject.setUrl(linkDetails.getFinalURL());
        linkObject.setContent(linkDetails.getRawContent());
        linkObject.setAdditionalProperty("linkDetails", linkDetails);
        if(!StringUtils.equals(linkDetails.getOriginalURL(), linkDetails.getFinalURL())) {
            Set<String> shortendUrls = Sets.newHashSet();
            shortendUrls.add(linkDetails.getOriginalURL());
            linkObject.setAdditionalProperty("alternateUrls", shortendUrls);
        }
        return linkObject;
    }

    private static final int BYTE_BUFFER_SIZE = 1024 * 50;
    public static final int MAX_ALLOWABLE_DOWNLOAD = 1024 * 1024 * 10;
    ...
    public void unwindLink(String url) {
        ...
        try {
            ...
            urlConnection.setConnectTimeout(10000); //TODO Make timeouts configurable
